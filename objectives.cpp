		...
		if (!objective->completed)
		{
			if (strcmp(objective->target, name) == 0)
			{
				...
				if (objective->currentValue == objective->targetValue)
				{
					...
					{
						sprintf(message, "%s - Objective Completed - Check Point Reached!", objective->description);
						game.setObjectiveCheckPoint();
					}
					else
					{
						sprintf(message, "%s - Objective Completed", objective->description);
					}
				...
				else if (!strstr(objective->target, "Combo-"))
				{
					if ((requiredValue % 10 == 0) || (requiredValue <= 10) || (alwaysInform))
					{
						...
						{
							case 0:
								sprintf(message, "%s - %d more to go...", objective->description, requiredValue);
								break;
							case 1:
								sprintf(message, "%s - need %d more", objective->description, requiredValue);
								break;
							case 2:
								sprintf(message, "%s - %d of %d", objective->description, objective->currentValue, objective->targetValue);
								break;
						}
