...
void skipBossMission()
{
	...
	if (game.stagesCleared == 10)
	{
		...
		graphics.delay(250);
		graphics.drawString("You got the Aqua Lung!", 320, 420, TXT_CENTERED, graphics.screen);
		graphics.delay(2000);
		SDL_FillRect(graphics.screen, NULL, graphics.black);
	}
	...
	if (game.stagesCleared == 15)
	{
		...
		SDL_FillRect(graphics.screen, NULL, graphics.black);
		graphics.delay(250);
		graphics.drawString("You got the Jet Pack!", 320, 420, TXT_CENTERED, graphics.screen);
		graphics.delay(2000);
		SDL_FillRect(graphics.screen, NULL, graphics.black);
	}
...
void showMissionClear()
{
	...
	if (map.totalMIAs > 0)
	{
		...
		else
		{
			...
			graphics.drawString("Completed", col2, y, TXT_LEFT, graphics.background);
		}
	}
	...
	while (objective->next != NULL)
	{
		...
		{
			graphics.drawString("???? ???????? ????", col1, y, TXT_RIGHT, graphics.background);
		}
		else
		{
			graphics.drawString(objective->description, col1, y, TXT_RIGHT, graphics.background);
		}
		...
		if (objective->currentValue < objective->targetValue)
		{
			...
			{
				graphics.setFontColor(0xff, 0x00, 0x00, 0x00, 0x00, 0x00);
				graphics.drawString("Incomplete", col2, y, TXT_LEFT, graphics.background);
			}
			else
			{
				graphics.setFontColor(0xff, 0x00, 0x00, 0x00, 0x00, 0x00);
	...
	while (true)
	{
		...
		sprintf(message, "%s - %.2d:%.2d:%.2d", "Mission Time", game.currentMissionHours, game.currentMissionMinutes, game.currentMissionSeconds);
