...
void droidBossChangeParticles()
{
	...
	for (int i = 0 ; i < 20 ; i++)
	{
		...
		dy /= 100;
		map.addParticle(self->x + (rand() % self->width), self->y  + (rand() % self->height), dx, dy, Math::rrand(1, 60), color, NULL, PAR_WEIGHTLESS);
	}
...
void droidBossDie()
{
	...
	addExplosion(self->x + rand() % self->width, self->y + rand() % self->height, 50, &player);
