...
void createStatsPanel(int page)
{
	...
	graphics.drawString("Metal Blob Solid : Statistics", 200, y, true, image);
	y += 10;
	...
	{
		case 0:
			graphics.drawString("Skill Level:", x1, y += 20, false, image);
			sprintf(string, "%s", skill[game.skill]);
			graphics.drawString(string, x2, y, false, image);

			graphics.drawString("Score:", x1, y += 20, false, image);
			sprintf(string, "%d", game.score);
			graphics.drawString(string, x2, y, false, image);

			graphics.drawString("MIAs Saved:", x1, y += 20, false, image);
			sprintf(string, "%d", game.totalMIAsRescued);
			graphics.drawString(string, x2, y, false, image);

			graphics.drawString("Objectives Completed:", x1, y += 20, false, image);
			...
			if (game.skill < 3)
			{
				graphics.drawString("Percentage Complete:", x1, y += 20, false, image);
				sprintf(string, "%d%%", gameData.getPercentageComplete());
				graphics.drawString(string, x2, y, false, image);
			}

			sprintf(string, "%s", "Total Game Time");
		...
		case 1:
			...
			graphics.drawString("Enemies Defeated:", x1, y += 20, false, image);
			sprintf(string, "%d", game.totalEnemiesDefeated);
			graphics.drawString(string, x2, y, false, image);

			graphics.drawString("Items Collected:", x1, y += 20, false, image);
			sprintf(string, "%d", game.totalItemsCollected);
			graphics.drawString(string, x2, y, false, image);

			graphics.drawString("Bonuses Picked Up:", x1, y += 20, false, image);
			sprintf(string, "%d", game.totalBonusesCollected);
			graphics.drawString(string, x2, y, false, image);

			graphics.drawString("Best Combo:", x1, y += 20, false, image);
			sprintf(string, "%d Hits", game.maxComboHits);
			graphics.drawString(string, x2, y, false, image);
			
			graphics.drawString("Missions Started:", x1, y += 20, false, image);
			sprintf(string, "%d", game.levelsStarted);
			graphics.drawString(string, x2, y, false, image);

			graphics.drawString("Continues Used:", x1, y += 20, false, image);
			sprintf(string, "%d", game.continuesUsed);
			graphics.drawString(string, x2, y, false, image);
			
			if ((game.continuesUsed > 0) && (game.levelsStarted > 0))
			{
				graphics.drawString("Average Continue Usage:", x1, y += 20, false, image);
				sprintf(string, "%d", (game.continuesUsed / game.levelsStarted));
				graphics.drawString(string, x2, y, false, image);
			}

			graphics.drawString("Escapes Used:", x1, y += 20, false, image);
		...
		case 2:
			...
			graphics.drawString("Weapon", x1, y += 20, false, image);
			graphics.drawString("Ammo Used", x2, y, false, image);
			graphics.drawString("Accuracy", x3, y, false, image);

			graphics.drawString("Pistol", x1, y += 30, false, image);
...
void loadLevelBrief(char *levelName)
{
	char briefLine[1024];

	if (!engine.loadData("data/levelBrief"))
	{
		graphics.showErrorAndExit("Couldn't load mission briefing file (%s)", "data/levelBrief");
	}
...
void createObjectivesPanel(char *levelName)
{
	SDL_Surface *image = graphics.getSprite("infoPanel", true)->image[0];
	...
	sprintf(string, "Information for %s", levelName);
	...
	while (data->next != NULL)
	{
		...
		if ((strcmp(data->key, levelName) == 0) || (strstr(data->key, levelMIAKey)))
		{
			...
			if (strstr(data->key, "MIAs"))
			{
				...
				{
					graphics.drawString("MIAs Rescued", x1, y += 20, false, image);
			...
			else if (!strstr(data->value, "MIA_"))
			{
				...
				{
					graphics.drawString("???? ???????? ????", x1, y += 20, false, image);
				}
				else
				{
					strcpy(string, data->value);
					
					if (strlen(data->value) >= 25)
					{
						string[25] = '.';
						string[26] = '.';
						string[27] = '.';
				...
				if (data->target == 1)
				{
					sprintf(string, "%s", (data->isComplete()) ? "Completed" : "Incomplete");
				}
				else
				{
					sprintf(string, "%d / %d", data->current, data->target);
...
void createMIAPanel(int start, int max)
{
	...
	graphics.drawString("MIA Statistics", 200, y, true, image);

	graphics.drawString("Name", x1, y += 20, false, image);
	graphics.drawString("Location", x2, y, false, image);
	graphics.drawString("Status", x3, y, false, image);
	...
	while (data->next != NULL)
	{
		...
		if (strstr(data->value, "MIA_"))
		{
			...
			graphics.drawString(data->key, x2, y, false, image);

			if (data->isComplete())
			{
				sprintf(string, "%s", "Found");
...
bool requirementMet(char *requiredLevel)
{
...
int doHub()
{
	...
	while (true)
	{
		...
		if (requirementMet(requiredLevel))
		{
			if (!gameData.levelPrefectlyCleared(name))
			{
				...
				hubPoint->levelNameImage = graphics.getString(name, false);
	...
	while (rtn == -1)
	{
		...
		if (validStage)
		{
			...
			sprintf(string, "%s", "Selected Destination");
			sprintf(string, "%s : %s", string, game.stageName);
		...
		mouseXDelta = engine.getJoyX(4);
		mouseYDelta = engine.getJoyY(4);

		if (engine.isKey(KEY_RIGHT))
		{
			mouseXDelta = 3;
		}
		
		if (engine.isKey(KEY_LEFT))
		{
			mouseXDelta = -3;
		}
		...
		while (hubPoint->next != NULL)
		{
			...
			if (Collision::collision(engine.getMouseX(), engine.getMouseY(), 1, 1, hubPoint->x, hubPoint->y, 16, 16))
			{
				...
				if (engine.mouseLeft || engine.isKey(KEY_FIRE))
				{
					game.setMapName(hubPoint->filename);
					game.setStageName(hubPoint->stageName);
					validStage = true;
					engine.mouseLeft = 0;
					engine.clearKey(KEY_FIRE);
				}

				if (engine.mouseRight || engine.isKey(KEY_MAP))
				{
					game.setMapName(hubPoint->filename);
					game.setStageName(hubPoint->stageName);
