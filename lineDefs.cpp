...
void addLineDef(char *name, char *linkName, char *message, int x, int y, int width, int height, bool active)
{
...
void showMessageLineDef(char *linkName, bool show)
{
	...
	while (lineDef->next != NULL)
	{
		...
		if ((absX < 800) && (absY < 600))
		{
			...
			if (Collision::collision(player.x + player.dx, player.y + player.dy, player.width, player.height, lineDef->x, lineDef->y, lineDef->width, lineDef->height))
			{
				...
				{
					engine.clearKey(KEY_LEFT);
					engine.clearKey(KEY_RIGHT);
