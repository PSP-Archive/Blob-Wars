...
void showMap(int centerX, int centerY)
{
	...
	graphics.drawString("Press Button to Continue...", 320, 450, TXT_CENTERED, graphics.screen);
	...
	while (true)
	{
		...
		if ((engine.isKey(KEY_MAP)) || (engine.isKey(KEY_PAUSE)) || (engine.keyState[SDLK_ESCAPE]))
		{
			break;
		}
