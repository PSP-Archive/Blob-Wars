...
void showInGameOptions()
{
	...
	while (true)
	{
		...
		if ((engine.isKey(KEY_PAUSE)) || (engine.keyState[SDLK_ESCAPE]))
		{
			engine.keyState[SDLK_ESCAPE] = 0;
			engine.clearKey(KEY_PAUSE);
			break;
		}
...
void showMissionInformation()
{
	...
	if (map.totalMIAs > 0)
	{
		...
		else
		{
			...
			graphics.drawString("Completed", col2, y, TXT_RIGHT, panel);
		}
	}
	...
	while (objective->next != NULL)
	{
		...
		{
			graphics.drawString("???? ???????? ????", col1, y, TXT_LEFT, panel);
		}
		else
		{
		...
		{
			graphics.setFontColor(0xff, 0x00, 0x00, 0x00, 0x00, 0x00);
			graphics.drawString("Incomplete", col2, y, TXT_RIGHT, panel);
		}
	...
	if (game.skill == 3)
	{
		...
		sprintf(message, "Time Limit - %d:%.2d Minutes", map.remainingMinutes, map.remainingSeconds);
		graphics.drawString(message, 200, 260, TXT_CENTERED, panel);
	}
	
	graphics.setFontColor(0xff, 0xff, 0xff, 0x00, 0x00, 0x00);
	graphics.drawString("Press Fire to Continue", 200, 280, TXT_CENTERED, panel);
